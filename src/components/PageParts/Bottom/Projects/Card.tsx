export default function Card({ project, index }: { project: any, index: number }) {
	...
	project.stacks[0] === "lua" ? classStacks = "w-[15%] md:w-[30%] object-contain" : classStacks = "w-[12%] object-contain"

	const isEven = index % 2 === 0;

	return (
		<a href={project.link} className="flex flex-col justify-between items-center bg-wine border-[#C6AF87] hover:bg-wine/80 border-solid border-4 rounded-lg shadow md:flex-row md:w-[90%] md:h-[7%]">
			{isEven ? (
				<img className="object-contain w-full rounded-t-lg md:h-full md:w-[40%] md:rounded-none md:rounded-s-lg" src={project.image} alt={project.name} />
			) : null}
			...
			{
				!isEven ? (
					<img className="object-cover w-full rounded-t-lg md:h-full md:w-[40%] md:rounded-none md:rounded-s-lg" src={project.image} alt={project.name} />
				) : null
			}
		</a >
	);
