...
var N=${JSON.stringify(WELCOME_CHIME_NOTES)},G=${WELCOME_CHIME_BASE_GAIN},F=${DECAY_FLOOR_RATIO},L=${SCHEDULE_LEAD_SECONDS},D=${WELCOME_CHIME_SECONDS};
...
function isBooting(){try{return sessionStorage.getItem(${JSON.stringify(BOOT_SEEN_STORAGE_KEY)})!==${JSON.stringify(BOOT_SEEN_VALUE)}}catch(e){return true}}
function chime(c){if(!isBooting())return;var v=readVolume();if(v<=0)return;var t=c.currentTime+L,i,o,g,p;
